...
class lj_settings {
    ...
    public function __construct() {
        if (is_admin()) {
            ...
            add_action("update_option_lj_setting_option", array($this, "add_setting_stylesheet"), 10, 2);
        }
    }

    public function add_setting_stylesheet($old_val, $new_val) {
        $file_get = LJ_PDP . "/assets/css/style.backup.css";
        $file_put = LJ_PDP . "/assets/css/style.css";
    ...
    public function general_settings_like_color($args) {
        $btn_color = get_option("lj_setting_option");
        if ($btn_color == FALSE) {
            $btn_color = "#000";
        } else {
        ...
        ?>
        <input type="text" class="wp-color-picker" name="lj_setting_option[<?php echo $args['name'] ?>]" value="<?php echo $btn_color; ?>" >    
        <?php
    }
